...
function normalizeLeverage(showAlert = false) {
  const rawValue = String(leverageInput.value).trim();

  // 입력창을 비웠을 때는 다시 1로 채우지 않고, 계산값만 0으로 처리
  if (rawValue === "") {
    return 0;
  }

  let leverageValue = Number(rawValue);
  ...
  // 레버리지는 정수로만 사용
  leverageValue = Math.floor(leverageValue);

  if (leverageValue > MAX_LEVERAGE) {
    leverageValue = MAX_LEVERAGE;

    if (showAlert) {
      alert("레버리지는 최대 500까지 입력할 수 있습니다!");
    }
  }
...
// 레버리지 입력창 클릭 시 0이면 비워서 바로 새로 입력 가능
...
// 입력창에서 나갔을 때 비어 있으면 0으로 복구
leverageInput.addEventListener("blur", () => {
  if (String(leverageInput.value).trim() === "") {
    leverageInput.value = "0";
  }
